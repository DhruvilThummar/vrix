          ...
          <form onSubmit={handleSaveCMS} className="space-y-8">
            ...
            {activeTab === "hero-philosophy" && (
              <div className="space-y-6 animate-fade-in">
                ...
                {/* Homepage Layout sections builder */}
                <section className="bg-pure-white border border-slate-grey/25 p-8 shadow-sm space-y-8 rounded">
                  <div>
                    <h3 className="font-headline-md text-lg text-deep-navy uppercase">
                      Homepage layout collections & products
                    </h3>
                    <p className="text-[10px] text-slate-grey mt-0.5">Visually choose what items and collections are showcased on the live homepage grid.</p>
                  </div>

                  {/* 1. Featured Collections */}
                  <div className="border-t border-slate-grey/10 pt-6 space-y-4">
                    <div className="flex justify-between items-center">
                      <div>
                        <h4 className="font-label-caps text-xs text-deep-navy font-bold tracking-wider uppercase">Showcase Collections</h4>
                        <p className="text-[10px] text-slate-grey">Featured collection cards on homepage (up to 4)</p>
                      </div>
                      <select
                        onChange={(e) => {
                        ...
                        value=""
                        className="border border-slate-grey/30 bg-pure-white text-xs px-2.5 py-1.5 outline-none font-semibold text-deep-navy cursor-pointer"
                      >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {featuredCollections.map((cId) => {
                        ...
                        return (
                          <div key={cId} className="border border-slate-grey/20 p-3 bg-soft-linen/5 rounded flex flex-col justify-between gap-3 relative">
                            <button
                              type="button"
                            ...
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 relative bg-soft-linen rounded overflow-hidden flex-shrink-0">
                                {col?.image && <img src={col.image} alt={col.title} className="object-cover w-full h-full" />}
                              </div>
                              <div className="min-w-0">
                                <p className="text-xs font-semibold text-deep-navy truncate">{col?.title || "Unknown Collection"}</p>
                                <p className="text-[9px] text-slate-grey truncate">{col?.tagline || "No tagline"}</p>
                              </div>
                            </div>
                          </div>
                        );
                      ...
                      {featuredCollections.length === 0 && (
                        <p className="col-span-4 text-center py-6 text-xs text-slate-grey/50 italic">No collections selected. Fallback to all collections.</p>
                      )}
                    </div>
                  </div>

                  {/* 2. New Arrivals Selector */}
                  <div className="border-t border-slate-grey/10 pt-6 space-y-4">
                    <div className="flex justify-between items-center">
                      <div>
                        <h4 className="font-label-caps text-xs text-deep-navy font-bold tracking-wider uppercase">New Arrivals Showcase</h4>
                        <p className="text-[10px] text-slate-grey">Dynamic products showcased in New Arrivals section (up to 4)</p>
                      </div>
                      <select
                        onChange={(e) => {
                          const val = e.target.value;
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {newArrivals.map((pId) => {
                        ...
                        return (
                          <div key={pId} className="border border-slate-grey/20 p-3 bg-soft-linen/5 rounded flex flex-col justify-between gap-3 relative">
                            <button
                              type="button"
                            ...
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 relative bg-soft-linen rounded overflow-hidden flex-shrink-0">
                                {prod?.image && <img src={prod.image} alt={prod.title} className="object-cover w-full h-full" />}
                              </div>
                              <div className="min-w-0">
                                <p className="text-xs font-semibold text-deep-navy truncate">{prod?.title || "Unknown Product"}</p>
                                <p className="text-[9px] text-slate-grey font-semibold">${prod?.price || 0}</p>
                              </div>
                            </div>
                          </div>
                        );
                      ...
                      {newArrivals.length === 0 && (
                        <p className="col-span-4 text-center py-6 text-xs text-slate-grey/50 italic">No products selected. Fallback to default lists.</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-grey/10 pt-6 space-y-4">
                    <div className="flex justify-between items-center">
                      <div>
                        <h4 className="font-label-caps text-xs text-deep-navy font-bold tracking-wider uppercase">Featured Products Showcase</h4>
                        <p className="text-[10px] text-slate-grey">Products showcased in Featured section (up to 4)</p>
                      </div>
                      <select
                        onChange={(e) => {
                          const val = e.target.value;
